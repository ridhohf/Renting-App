...
export class AuthService {
  async register(data: { email: string; name: string; role: any }) {
    ...
    const html = AuthEmailHelper.buildVerificationEmail(user.name, `${ENV.CLIENT_URL}/verify?token=${token}`);
  ...
  async verifyAndSetPassword(token: string, password?: string) {
    ...
    if (!user || user.verificationToken !== token) throw new AppError('Invalid or expired token', 400);

    const updateData: any = { isVerified: true, verificationToken: null };
  ...
  async login(email: string, pass: string) {
    ...
    if (user.provider === 'GOOGLE') throw new AppError('Please use Google login for this account', 400);
  ...
  async resetPassword(token: string, newPassword: string) {
    ...
    return prisma.user.update({ where: { id: user.id }, data: { passwordHash, resetToken: null } });
  }
  ...
  async googleLogin(googleToken: string) {
    ...
    if (!user) {
      user = await prisma.user.create({
        data: { email: payload.email, name: payload.name || 'User', provider: 'GOOGLE', isVerified: true, role: 'USER' },
      });
    }
    const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, ENV.JWT_SECRET, { expiresIn: '7d' });
    const { passwordHash, ...userData } = user;
